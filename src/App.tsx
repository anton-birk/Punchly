...
function App() {
	...
	const handleStopTimer = async () => {
		...
		try {
			await logTime(
				settings,
				result.timer.workPackageId,
				result.timer.projectId,
				result.elapsed,
				result.timer.comment,
				today,
			)
			setLogError('')
		} catch (e) {
					...
					<main className="app-main flex-1 flex flex-col overflow-hidden">
						{updateAvailable && (
							<div className="brand-soft brand-border flex items-center justify-between gap-3 border-b text-xs px-5 py-2">
								...
								<button
									...
									className="link-accent transition-colors cursor-pointer"
								>
									<X size={14} />
								</button>
							</div>
						...
						{logError && (
							<div className="danger-chip danger-border flex items-center justify-between gap-3 border-b text-xs px-5 py-2">
								<span>{logError}</span>
								<button onClick={() => setLogError('')} className="hover:opacity-80">
									<X size={14} />
								</button>
							</div>
						)}
