...
export async function updateWorkPackage(
	...
	return await invoke('update_work_package', {
		url: settings.url,
		apiKey: settings.apiKey,
		id,
		data,
...
async function getMembersAsAssignees(settings: Settings, projectId: number): Promise<Assignee[]> {
	...
	try {
		const res: any = await invoke('get_project_assignees', {
			url: settings.url,
			apiKey: settings.apiKey,
			projectId,
		})
	...
	try {
		const res: any = await invoke('get_project_members', {
			url: settings.url,
			apiKey: settings.apiKey,
			projectId,
		})
...
export async function getProjectWpForm(
	settings: Settings,
	projectId: number,
): Promise<{ statuses: Status[]; assignees: Assignee[] }> {
	// Try the form endpoint first (requires add_work_packages permission)
	try {
		const form: any = await invoke('get_project_wp_form', {
			url: settings.url,
			apiKey: settings.apiKey,
			projectId,
		})
		...
		const assigneeLinks: { href: string; title: string }[] =
			schema?.assignee?._links?.allowedValues ?? []
		let assignees: Assignee[] = assigneeEmbedded.length
			? assigneeEmbedded
			: assigneeLinks.map((l) => ({
					id: parseInt(l.href.split('/').pop() ?? '0', 10),
					name: l.title,
				}))
...
export async function getProjectVersions(
	...
	try {
		const res: any = await invoke('get_project_versions', {
			url: settings.url,
			apiKey: settings.apiKey,
			projectId,
		})
...
export async function getTimeEntries(
	settings: Settings,
	workPackageId: number,
): Promise<ApiCollection<TimeEntry>> {
	return await invoke('get_time_entries', {
		url: settings.url,
		apiKey: settings.apiKey,
		workPackageId,
	})
