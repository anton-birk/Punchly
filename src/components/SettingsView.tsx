...
export function SettingsView({ settings, onSave }: Props) {
	...
	useEffect(() => {
		...
		invoke<boolean>('accessibility_granted')
	...
	const connectionDirty = url.trim() !== settings.url || apiKey.trim() !== settings.apiKey

	const idleDirty =
		idleEnabled !== settings.idleEnabled || idleThresholdMin !== settings.idleThresholdMin
	...
	const inputCls =
		'input-field w-full border rounded-md px-3 py-2 text-sm outline-none transition-colors'
	...
	const SaveButton = ({
		...
		const justSaved =
			savedSection === section && (section === 'connection' ? !connectionDirty : !idleDirty)
		return (
			<button
	...
	return (
		<div className="h-full overflow-y-auto">
			...
			<div className="surface mx-6 mt-4 max-w-lg border rounded-xl p-6">
				...
				<label className="flex items-center gap-3 cursor-pointer mb-4">
					<div className="relative">
						...
						/>
						<div
							className={`w-10 h-6 rounded-full transition-colors ${idleEnabled ? 'bg-[var(--brand)]' : 'bg-[var(--app-border-strong)]'}`}
						/>
						<div
							className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform ${idleEnabled ? 'translate-x-5' : 'translate-x-1'}`}
						/>
					</div>
					...
					<div className="mb-4">
						...
						<div className="flex items-center gap-3">
							<input
								...
								onChange={(e) =>
									setIdleThresholdMin(Math.max(1, Math.min(60, Number(e.target.value))))
								}
						...
						<p className="text-subtle mt-1.5 text-xs">
							After this period without mouse/keyboard activity, Punchly will ask whether to include
							the idle time when you return.
						</p>
					</div>
				)}
			...
			{isMacOS && (
				<div className="surface mx-6 mt-4 max-w-lg border rounded-xl p-6">
					...
					<p className="text-subtle text-xs mb-5">
						Allows Punchly to track keyboard and mouse events to precisely measure idle time — the
						same approach used by HubStaff. Without this, detection relies on polling which may miss
						idle periods after screen lock.
					</p>
					...
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-2">
							...
							<span className="text-muted text-sm">
								...
								{accessibilityGranted === true &&
									'Accessibility granted — event-based idle tracking active'}
								{accessibilityGranted === false &&
									'Accessibility not granted — using polling fallback'}
							</span>
						</div>
						{accessibilityGranted === false && (
							<button
			...
			<div className="surface mx-6 mt-4 mb-6 max-w-lg border rounded-xl p-6">
				...
				<p className="text-subtle text-xs mb-5">
					Punchly shows a system notification when idle time is detected — this works even when the
					app is in another Space or behind other windows.
				</p>
				...
				<div className="flex items-center justify-between">
					...
					{notifGranted === true && (
						<button
							onClick={() =>
								sendNotification({ title: 'Punchly', body: 'Test notification is working!' })
							}
				...
				{notifGranted === false && (
					<p className="text-subtle mt-3 text-xs">
						If the dialog doesn't appear, go to{' '}
						<span className="text-muted font-medium">
							System Settings → Notifications → Punchly
						</span>{' '}
