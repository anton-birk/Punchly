...
export function TasksView({ settings, onlyMine, currentUserId, runningWpId, onStartTimer, onStopTimer }: Props) {
  ...
  const selectCls = 'input-field h-9 border rounded-md px-2.5 text-sm outline-none hover:border-[var(--app-border-strong)] transition-colors cursor-pointer flex-shrink-0';
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <div className="divider flex items-center gap-2.5 px-6 py-3.5 border-b">
          <form onSubmit={handleSearch} className="flex gap-2 flex-1 min-w-0">
            ...
            />
            <button type="submit" className="btn-ghost h-9 px-3.5 text-sm font-medium rounded-md border transition-colors cursor-pointer whitespace-nowrap">
              Search
            </button>
          </form>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-3 flex flex-col gap-1.5">
          {loadError && (
            <div className="warning-chip warning-border flex items-center gap-2 text-xs border rounded-md px-3 py-2 mt-1">
              ...
              <button onClick={() => load(1)} className="underline hover:opacity-80 cursor-pointer whitespace-nowrap">Retry</button>
            </div>
          )}
          {loading && tasks.length === 0 && (
            <div className="text-subtle flex items-center justify-center py-16 text-sm">Loading…</div>
          )}
          {!loading && tasks.length === 0 && !loadError && (
            <div className="text-subtle flex items-center justify-center py-16 text-sm">No tasks found</div>
          )}
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-1">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-subtle text-[11px] ml-auto truncate max-w-[110px] flex-shrink-0">{projectTitle}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {wp.percentageDone > 0 && (
                      <span className="text-subtle text-[11px] flex-shrink-0">{wp.percentageDone}%</span>
                    )}
                    {spent.hours > 0 && (
