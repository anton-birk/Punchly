	...
	return (
		<aside className="sidebar-shell w-[230px] min-w-[230px] border-r flex flex-col">
			<div
				...
				className="bg-transparent h-7 cursor-grab active:cursor-grabbing"
			/>
			...
			{timerRunning && (
				<div
					className={`mx-3 mt-3.5 rounded-lg p-3.5 flex flex-col gap-1.5 border ${
						isIdle ? 'warning-chip warning-border' : 'success-chip success-border'
					}`}
				>
					...
					<div
						className={`tabular text-[26px] font-bold leading-none ${isIdle ? 'opacity-70' : ''}`}
					>
				...
				{nav.map(({ id, label, Icon }) => (
					<button
						...
						className={`flex items-center gap-2.5 w-full text-left px-2.5 py-2 rounded-md text-sm font-medium transition-colors cursor-pointer ${
							view === id ? 'nav-active' : 'nav-idle text-muted'
						}`}
					>
						<Icon size={16} />
						<span>{label}</span>
