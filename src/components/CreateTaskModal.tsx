	...
	useEffect(() => {
		Promise.all([getProjects(settings), getTypes(settings), getPriorities(settings)])
			.then(([p, t, pr]) => {
				...
				const defPriority =
					pr._embedded.elements.find((x) => x.isDefault) ?? pr._embedded.elements[0]
		...
		>
			...
			>
				<div className="divider flex items-center justify-between px-5 py-4 border-b">
					...
					<button
						onClick={onClose}
						className="text-subtle hover:text-[var(--app-text)] p-1 cursor-pointer"
					>
						<X size={16} />
					</button>
				</div>
				...
				<form onSubmit={handleSubmit} className="flex flex-col overflow-y-auto">
					<div className="p-5 flex flex-col gap-4">
							...
							{files.length > 0 && (
								<div className="flex flex-col gap-1.5">
									{files.map((file, i) => (
										<div
											key={i}
											className="surface-muted flex items-center gap-2 rounded-md px-3 py-2 text-sm"
										>
											<span className="text-subtle text-base">
											...
											<button
												...
												className="text-subtle hover:text-[var(--danger)] cursor-pointer ml-1"
											>
												<X size={13} />
											</button>
										</div>
						...
						{error && (
							<div className="danger-chip danger-border text-xs border rounded-md p-3 break-all">
								{error}
							</div>
						)}
					</div>
					...
					<div className="divider flex justify-end gap-2.5 px-5 py-4 border-t">
						<button
							type="button"
							onClick={onClose}
							className="btn-ghost px-4 py-2 text-sm font-medium rounded-md border transition-colors cursor-pointer"
						>
							Cancel
						</button>
						<button
							type="submit"
							disabled={!subject.trim() || !projectId || !typeId || saving}
							className="btn-primary px-4 py-2 text-sm font-semibold rounded-md disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
						>
